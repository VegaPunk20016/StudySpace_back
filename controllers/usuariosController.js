  ...
  try {
    ...
    if (!nombre || !correo || !rol || !password) {
      return res
        ...
        .json({
          success: false,
          message: "nombre, correo, rol y password son requeridos",
        });
    }
...
async function login(req, res) {
  try {
    ...
    if (!correo || !password) {
      return res
        .status(400)
        .json({ success: false, message: "email y password son requeridos" });
    }
    ...
    const token = jwt.sign(
      ...
      { expiresIn: process.env.JWT_EXPIRATION } // duración
    );
    ...
    res.json({
      ...
      user: Mapper.mapUsuario(usuario),
    });
